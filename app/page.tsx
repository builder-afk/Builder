...
export default function BuildersPage() {
  ...
  const trendingProjects = useMemo(() => {
    // Strictly deduplicate by videoUrl and project id so no video or project is ever repeated
    ...
    const uniqueProjects = allProjects.filter((p) => {
      ...
      seenIds.add(p.id);
      if (p.videoUrl) {
        if (seenVideos.has(p.videoUrl)) return false;
        seenVideos.add(p.videoUrl);
      }
      return true;
    });

    let projects = [...uniqueProjects].sort((a, b) => b.views - a.views);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-body">
      ...
      <div className="section-container pb-24">
        ...
        {/* ─── BEFORE / AFTER VIRTUAL STAGING COMPARISON SLIDERS ─── */}
        <VirtualStagingShowcase />

        {/* ─── MULTI-STYLE VIRTUAL STAGING SHOWCASE (FLUID SWITCHER) ─── */}
