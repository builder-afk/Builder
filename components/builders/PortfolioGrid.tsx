...
import { X, Heart, MapPin, Eye, Building2, ChevronLeft, ChevronRight, Play, Film } from "lucide-react";
  ...
  return (
    ...
    >
      <div className={`relative ${height} bg-gradient-to-br ${project.gradient} rounded-[1.25rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300`}>
        ...
        <div className="absolute top-3 left-3 flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/85 backdrop-blur-md text-[11px] font-medium text-slate-700 shadow-sm">
          {project.videoUrl && <Film className="w-3 h-3 text-[#F26522]" />}
          {project.type}
        </div>
      </div>
    </motion.div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Project Visual */}
        <div className={`h-72 sm:h-96 bg-gradient-to-br ${project.gradient} relative overflow-hidden flex items-center justify-center bg-black`}>
          {project.videoUrl ? (
            <video
              src={project.videoUrl}
              autoPlay
