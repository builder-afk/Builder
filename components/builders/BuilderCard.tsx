  ...
  if (variant === "horizontal") {
    return (
      ...
      >
        ...
        >
          {/* Full-width Portfolio Image (Architectural Transformation Photo) */}
          <div className={`relative h-44 bg-gradient-to-br ${builder.coverGradient} overflow-hidden`}>
            {builder.houseTransformation?.stages && builder.houseTransformation.stages.length > 0 ? (
              <img
                src={builder.houseTransformation.stages[builder.houseTransformation.stages.length - 1].imageUrl}
                alt={builder.name}
            ...
            />
            {/* Top Transform Badge */}
            <div className="absolute top-2.5 left-2.5 bg-black/60 backdrop-blur-md rounded-full px-2.5 py-1 text-[10px] font-semibold text-white/90 shadow-sm border border-white/20">
